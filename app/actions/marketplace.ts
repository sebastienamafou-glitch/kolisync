...
export async function claimOpportunityAction(orderId: string) {
  ...
  if (!session || session.role !== "DRIVER") {
    throw new Error("Seuls les livreurs peuvent accepter des opportunités.");
  }
  ...
  return await prismaAdmin.$transaction(async (tx) => {
    ...
    // 🚨 Utilisation stricte de la caution (depositAmount) au lieu de amountDue
    ...
    // On ne crée une transaction de gel que si la caution exigée est supérieure à 0
